  ...
  const makePayment = async () => {
    ...
    try {
      const response = await fetch(
        "https://barista-coffee-server.vercel.app/api/create-checkout-session",
        {
          method: "POST",
          headers: headers,
          ...
          <div className="w-full h-full flex flex-col items-center justify-start overflow-y-auto text-white">
            <div className="flex h-auto flex-col w-full p-4">
              <table className="w-full table-auto">
                <thead
                  className={`${isModalVisible ? "text-right" : "text-center"}`}
                >
                  ...
                  <tbody className="text-black text-sm" key={product.id}>
                    <tr className="text-center bg-lightish border border-b-2">
                      ...
                      <td className="px-4 py-2">
                        <button
                          className="px-3 py-0 border border-black md:hover:bg-dark-brown md:hover:text-white"
                          onClick={() => handleDecreaseQuantity(index)}
                        >
                          -
                        </button>
        ...
        {cartProducts.length > 0 && (
          <div>
            <div className="flex flex-col md:flex-row items-center justify-between w-full my-4 gap-2 md:px-4 px-0 text-sm">
              ...
              />
              <button
                className="button cursor-pointer w-full md:w-80 px-1 py-3 bg-lightish hover:bg-dark-brown hover:text-white md:hover:shadow-md"
                onClick={couponCode}
              >
                Apply Discount
              </button>
